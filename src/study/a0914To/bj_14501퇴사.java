...
public class bj_14501퇴사 {
	...
	public static void main(String[] args) throws IOException {
		...
		for(int i = N; i>=1; i--) { //거꾸로부터 해야 구할수 있??,, N+1를 0으로 초기화
				if(i+ T[i]>N+1) {
					P[i] = P[i+1]; //N+1일째에는 회사에 없기 때문에... 날짜 범위.  
		...
		/*for(int i = 1; i<=N; i++) { //거꾸로부터 해야 구할수 있??,, N+1를 0으로 초기화
			if(i+ T[i]<N+1) {
				P[i+1] = P[i]; //N+1일째에는 회사에 없기 때문에... 날짜 범위.  
				//(거꾸로부터 반복문을 돌렸으므로) 이전값이 들어감.
			} else {
				P[i] = Math.max(P[i+1], P[i]+P[i+T[i]]);// 이전값, 현재값+현재값에서 시간더한번째값.
				//굳이 치환?->중간 빈 날짜 있는 경우 고려?
